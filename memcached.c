...
int main (int argc, char **argv) {
    ...
    srand(time(NULL));  
    rtd = (int*)malloc(sizeof(int)*MAXL);
    ...
    gap = (int*)malloc(sizeof(int)*(MAXC+1));
    memset(gap,0,sizeof(int)*(MAXC+1));
...
int domain_index_to_value(int index)
{
    int value = 0,step = 1;
    while (index>domain) {
        value += step*domain;
...
void insert(int clsid, int now)
{
    insnum[clsid]++;
    int loc = qlen[clsid];
    if (qlen[clsid]==MAXH) {
        loc = rand()%insnum[clsid];
...
void find(int clsid, int now)
{
    int t = now%MAXH;
    struct node *tmp = hash_t[clsid][t];
    while (tmp) {
        if (tmp->addr==now) {
            if (tmp->ed==-1) tmp->ed = ref[clsid];
            return;
        }
        tmp = tmp->nxt;
    }
