...
int main (int argc, char **argv) {
	...
	/*rtd = (long long*)malloc(sizeof(long long)*MAXL);
	memset(rtd,0,sizeof(long long)*MAXL);
	ref = (int*)malloc(sizeof(int)*(MAXC+1));
	memset(ref,0,sizeof(int)*(MAXC+1));
	miss_ref = (int*)malloc(sizeof(int)*(MAXC+1));
	memset(miss_ref,0,sizeof(int)*(MAXC+1));
	gap = (long long*)malloc(sizeof(long long)*(MAXC+1));
	memset(gap,0,sizeof(long long)*(MAXC+1));
	touch = (int*)malloc(sizeof(int)*(MAXC+1));
	memset(touch,0,sizeof(int)*(MAXC+1));
	total_slab = settings.maxbytes/(1024*1024);
	srand(time(NULL));  
	insnum = (int*)malloc(sizeof(int)*(MAXC+1));
	memset(insnum,0,sizeof(int)*(MAXC+1));
	qlen = (int*)malloc(sizeof(int)*(MAXC+1));
	memset(qlen,0,sizeof(int)*(MAXC+1));
	hash_t = (struct node*)malloc(sizeof(struct node*)*(MAXC+1)*(MAXH));
	for(int i=0; i<=MAXC; i++)
	{
		tar[i] = (struct node*)malloc(sizeof(struct node)*MAXH);
	}*/
    /*
...
void insert(int clsid, long long now)
{
    insnum++;
    int loc = qlen[clsid];
    if (qlen[clsid]==MAXH) {
        loc = rand()%insnum;
        if (loc>=qlen[clsid]) return;
        if (tar[clsid][loc].pre==NULL && tar[clsid][loc].nxt==NULL) {
            hash_t[clsid][tar[clsid][loc].addr%MAXH] = NULL;
        }
        else if (tar[clsid][loc].pre==NULL) {
            hash_t[clsid][tar[clsid][loc].addr%MAXH] = tar[clsid][loc].nxt;
            tar[clsid][loc].nxt->pre = NULL;
        }
        else {
            tar[clsid][loc].pre->nxt = tar[clsid][loc].nxt;
            if (tar[clsid][loc].nxt) tar[clsid][loc].nxt->pre = tar[clsid][loc].pre;
        }
    }
    else qlen[clsid]++;
    int t = now%MAXH;
    node *tmp = tar[clsid]+loc;
    tmp->addr = now;
    tmp->label = ref;
    tmp->nxt = hash_t[clsid][t];
    tmp->ed = -1;
    if (hash_t[clsid][t]) hash_t[clsid][t]->pre = tmp;
    tmp->pre = NULL;
    hash_t[clsid][t] = tmp;
}

void find(int clsid, long long now)
{
    int t = now%MAXH;
    node *tmp = hash_t[clsid][t];
    while (tmp) {
        if (tmp->addr==now) {
            if (tmp->ed==-1) tmp->ed = ref;
            return;
        }
        tmp = tmp->nxt;
    }
