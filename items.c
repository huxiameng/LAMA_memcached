...
item *do_item_alloc(char *key, const size_t nkey, const int flags,
    ...
    it->nsuffix = nsuffix;
    if (it&&settings.slab_automove == 1) 
    {
	pthread_mutex_lock(&movelock);
	ref[it->slabs_clsid]++;
	miss_ref[it->slabs_clsid]++;
	find(it->slabs_clsid,atoi(key));
	touch[it->slabs_clsid] = 1;
	if(ref[it->slabs_clsid]==gap[it->slabs_clsid])
	{
		insert(it->slabs_clsid,atoi(key));
		gap[it->slabs_clsid] += rand()%(STEP*2)+1;
	}
